...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'status_update') {
        ...
        if (in_array($status, $allowed, true)) {
            ...
            $sql = 'UPDATE parcels SET status = ?' . $setDeliveredAt . $setFailedReason . ' WHERE id = ? AND assigned_rider_id = ?';
            $stmt = db()->prepare($sql);
            $params = [$status];
            ...
            $stmt->execute([$parcelId, $rider['id'], $status, $remarks, $latitude, $longitude]);
...
$photos = db()->prepare('SELECT * FROM delivery_photos WHERE parcel_id = ? ORDER BY created_at DESC');
